...
for (const v of REQUIRED_FALLBACK_VARS) {
  if (!process.env[v]) {
    console.warn(
      `[analyse-company] WARNING: environment variable ${v} is not set — the corresponding fallback provider will be unavailable`
    );
  }
}
...
/**
 * Core analysis pipeline for a single company.
 *
 * 1. Cache check — returns immediately if a fresh result exists
 * 2. Fetch filing history from Companies House
 * 3. Find the most recent accounts filing with a downloadable document
 * 4. Download the PDF
 * 5. Try Anthropic → Gemini → Mistral → Google Document AI
 * 6. Parse, store in cache, and return
 *
 * Throws AnalyseError on failure.
 */
export async function analyseCompany(
  companyNumber: string
): Promise<AccountsAnalysis | null> {
  const cn = companyNumber.toUpperCase();

  // ── 1. Cache check ──────────────────────────────────────────────────────────
  const cached = await getCachedAnalysis(cn);
  ...
  try {
    const filingRes = await fetch(
      `${CH_BASE}/company/${cn}/filing-history?items_per_page=50&category=accounts`,
      { headers: { Authorization: chAuth() }, cache: "no-store" }
    );
    if (!filingRes.ok) {
      throw new AnalyseError(
        `Filing history fetch failed (${filingRes.status})`,
        filingRes.status
      );
    }
    ...
    if (err instanceof AnalyseError) throw err;
    throw new AnalyseError("Network error fetching filing history", 502);
  }

  // ── 3. Most recent accounts filing ─────────────────────────────────────────
  ...
  if (!filing) {
    throw new AnalyseError("No accounts document found for this company", 404);
  }

  console.log(
    `[analyse-company][diag] ${cn}: selected filing type: ${filing.type ?? "unknown"} | date: ${filing.date ?? "unknown"}`
  );
  ...
  if (!idMatch) {
    throw new AnalyseError(
      "Could not extract document ID from filing link",
      500
    );
  }
  ...
  try {
    ...
    if (!pdfRes.ok) {
      throw new AnalyseError(
        `Document download failed (${pdfRes.status})`,
        pdfRes.status
      );
    }
    ...
    if (arrayBuffer.byteLength > MAX_PDF_BYTES) {
      throw new AnalyseError("Document exceeds 20 MB size limit", 413);
    }

    pdfBuffer = Buffer.from(arrayBuffer);
    pdfBase64 = pdfBuffer.toString("base64");
    console.log(
      `[analyse-company] ${cn}: downloaded ${Math.round(arrayBuffer.byteLength / 1024)} KB PDF`
    );
  } catch (err) {
    if (err instanceof AnalyseError) throw err;
    throw new AnalyseError("Network error downloading document", 502);
  }
    ...
    messages: [
      {
        ...
        content: [
          ...
          { type: "text", text: "Please analyse this company accounts document." },
        ],
      },
    ],
  });
  ...
  try {
    ...
    console.log(
      "[analyse-company] → Anthropic request body:",
      JSON.stringify(bodyForLogging, null, 2)
    );
  } catch {
    console.log(
      "[analyse-company] → Anthropic request body (raw):",
      anthropicBody.slice(0, 500)
    );
  }
  ...
  try {
    for (let attempt = 0; attempt < RETRY_DELAYS_MS.length; attempt++) {
      ...
      console.warn(
        `[analyse-company] ${cn}: Anthropic overloaded (529),` +
    ...
    if (anthropicRes.status === 400) {
      ...
      console.error("[analyse-company] Anthropic error:", body);
      throw new AnalyseError(
    ...
    if (err instanceof AnalyseError) throw err;
    throw new AnalyseError("Network error calling Anthropic API", 502);
  }

  // ── Fallback chain (triggered on Anthropic 400) ─────────────────────────────
  if (anthropicHit400) {
    console.log(`[analyse-company] ${cn}: Anthropic 400 — trying Gemini`);

    try {
      const geminiAnalysis = await analyseWithGemini(pdfBase64, cn, filing.date);
      await setCachedAnalysis(cn, geminiAnalysis);
      return geminiAnalysis;
    } catch (geminiError) {
      console.error(
        `[analyse-company] ${cn}: Gemini failed — trying Mistral`,
        geminiError
      );
    }

    try {
      const mistralAnalysis = await analyseWithMistral(pdfBase64, cn, filing.date);
      await setCachedAnalysis(cn, mistralAnalysis);
      return mistralAnalysis;
    } catch (mistralError) {
      console.error(
        `[analyse-company] ${cn}: Mistral failed — trying Google Document AI OCR`,
        mistralError
      );
    }

    try {
      const docaiAnalysis = await analyseWithDocAI(pdfBuffer, cn, filing.date);
      await setCachedAnalysis(cn, docaiAnalysis);
      return docaiAnalysis;
    } catch (docaiError) {
      console.error(
        `[analyse-company] ${cn}: Google Document AI OCR failed — all providers exhausted`,
        docaiError
      );
    }
  ...
  console.log(`[analyse-company] ${cn}: Claude responded (${rawText.length} chars)`);
  ...
  try {
    ...
    analysis = {
      ...
      auditorAndGoingConcern: parsed.auditorAndGoingConcern
        ? {
            ...
            goingConcernFlag: Boolean(parsed.auditorAndGoingConcern.goingConcernFlag),
            goingConcernDetail: parsed.auditorAndGoingConcern.goingConcernDetail ?? "",
            emphasisOfMatter: parsed.auditorAndGoingConcern.emphasisOfMatter ?? "",
          }
        : undefined,
    ...
    console.error(
      "[analyse-company] JSON parse failed. Raw text:",
      rawText.slice(0, 500)
    );
    throw new AnalyseError(
  ...
  // ── 7. Cache and return ─────────────────────────────────────────────────────
  await setCachedAnalysis(cn, analysis);
  return analysis;
}
